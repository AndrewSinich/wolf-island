	...
	for (int i = 1; i < 21; i++) {
		for (int k = 1; k < 21; k++) {
			for (int t = 0; t < bunnies.size(); t++) {
				if (bunnies[t].show_pos_x() == i && bunnies[t].show_pos_y() == k) {
					this->an[i][k].buns++;
				}
			}
			for (int t = 0; t < wolfess.size(); t++) {
				if (wolfess[t].show_pos_x() == i && wolfess[t].show_pos_y() == k) {
					this->an[i][k].girl_wolfies++;
				}
			}
			for (int t = 0; t < wolfs.size(); t++) {
				if (wolfs[t].show_pos_x() == i && wolfs[t].show_pos_y() == k) {
					this->an[i][k].wolfies++;
				}
			}
		}
...
void Map::die() {
	...
	for (int x = wolfess.size() - 1; x > -1; x--) {
		if (!wolfess[x].is_alive() || wolfess[x].show_health() == 0) {
			wolfess.erase(wolfess.begin() + x);
		}
	}
	for (int x = wolfs.size() - 1; x > -1; x--) {
		if (!wolfs[x].is_alive() || wolfs[x].show_health() == 0) {
...
void Map::move_wolfess() {
	...
	for (auto& k : wolfess) {
		if (k.is_alive()) {
			if (!this->sence_bunnies(k.show_pos_x(), k.show_pos_y()) || k.show_health() > 5)
				k.change_positions();
			else
				for (auto& i : bunnies) {
					if ((i.show_pos_x() == k.show_pos_x() || i.show_pos_x() == k.show_pos_x() + 1 || i.show_pos_x() == k.show_pos_x() - 1) && (i.show_pos_y() == k.show_pos_y() || i.show_pos_y() == k.show_pos_y() + 1 || i.show_pos_y() == k.show_pos_y() - 1))
...
void Map::move_wolfs() {
	for (auto& t : wolfs) {
		if (t.is_alive()) {
			if (this->sence_bunnies(t.show_pos_x(), t.show_pos_y()) && t.show_health() <= 4) {
				for (auto& i : bunnies) {
					if ((i.show_pos_x() == t.show_pos_x() || i.show_pos_x() == t.show_pos_x() + 1 || i.show_pos_x() == t.show_pos_x() - 1) && (i.show_pos_y() == t.show_pos_y() || i.show_pos_y() == t.show_pos_y() + 1 || i.show_pos_y() == t.show_pos_y() - 1))
						t.wolf_move(i.show_pos_x(), i.show_pos_y());
				}
			}

			else if (this->sence_wolfess(t.show_pos_x(), t.show_pos_y()) && t.show_health() > 4) {
				for (auto& i : wolfess) {
					if ((i.show_pos_x() == t.show_pos_x() || i.show_pos_x() == t.show_pos_x() + 1 || i.show_pos_x() == t.show_pos_x() - 1) && (i.show_pos_y() == t.show_pos_y() || i.show_pos_y() == t.show_pos_y() + 1 || i.show_pos_y() == t.show_pos_y() - 1))
						t.wolf_move(i.show_pos_x(), i.show_pos_y());
				}
...
bool Map::sence_bunnies(int x, int y) {
	for (auto& k : bunnies) {
		if ((k.show_pos_x() == x || k.show_pos_x() == x + 1 || k.show_pos_x() == x - 1) && (k.show_pos_y() == y || k.show_pos_y() == y + 1 || k.show_pos_y() == y - 1)) {
			return true;
		}
	}
	return false;
...
void Map::eat() {
	for (auto& i : wolfess) {
		if (i.is_alive()) {
			for (auto& k : bunnies) {
				if (k.show_pos_x() == i.show_pos_x() && k.show_pos_y() == i.show_pos_y()) {
					k.die();
					i.plus_health();
					break;
				}
...
void Map::bunny_birth() {
	for (auto& i : bunnies) {
		if (i.is_alive() && bunnies.size() < 400 && i.show_age() >= 2) {
			int t = rand() % 10;
			if (t == 0 || t == 1)
...
void Map::wolf_birth() {
	for (auto& i : wolfs) {
		if (i.show_age() >= 10) {
			for (auto& k : wolfess) {
				if (k.show_age() >= 10) {
					if (i.show_pos_x() == k.show_pos_x() && i.show_pos_y() == k.show_pos_y() && wolfess.size() < 400 && wolfs.size() < 400) {
						int t = rand() % 2;
						if (t == 0)
...
void Map::show_bunnies() {
	cout << "Зайцы: " << this->bunnies.size() << endl;
}
void Map::show_wolvess() {
	cout << "Волчицы: " << this->wolfess.size() << endl;
}
void Map::show_wolfs() {
	cout << "Волки: " << this->wolfs.size() << endl;
}
